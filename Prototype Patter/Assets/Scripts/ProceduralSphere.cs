...
public class ProceduralSphere : Object {

    private static GameObject sphere;

    public static GameObject GetSphere(Vector3 position) {
        if(sphere == null) {
            CreateSphere(Vector3.zero);
            sphere.SetActive(false);
        }

        GameObject clone = Instantiate(sphere, position, Quaternion.identity);
        clone.SetActive(true);
        return clone;

    }

    private static void CreateSphere(Vector3 pos) {
