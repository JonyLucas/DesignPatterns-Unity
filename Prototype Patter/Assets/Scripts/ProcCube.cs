...
public class ProcCube : Object {
    ...
    public static GameObject GetCube(Vector3 position) {
        if(cube == null) {
            CreateCube(Vector3.zero);
            cube.SetActive(false);
        }

        GameObject clone = Instantiate(cube, position, Quaternion.identity);
    ...
    private static void CreateCube(Vector3 pos) {
        ...
        filter.mesh.name = "Cube_" + Time.realtimeSinceStartup;
        MeshRenderer rederer = cube.GetComponent<MeshRenderer>();
        rederer.material = new Material(Shader.Find("Holistic/Plasma"));
